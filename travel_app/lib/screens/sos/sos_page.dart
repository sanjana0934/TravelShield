...
// ── Tokens — exactly from home_page.dart ──────────────────────────────────────
...
class _SOSPageState extends State<SOSPage> with TickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> _helplines = [
    {'category': 'Universal'},
    {
      ...
      'number': '112',
      'desc': 'Police, Fire & Ambulance combined',
    ...
    {'category': 'Medical'},
    {
      'name': 'Ambulance',
      'number': '108',
      'desc': 'Free emergency ambulance service',
    ...
    },
    {
      ...
      'number': '104',
      'desc': 'Health advice & medical guidance',
    ...
    },
    {
      'name': 'Blood Bank',
      'number': '1910',
      'desc': 'Emergency blood requirement',
    ...
    },
    {
      'name': 'AYUSH Helpline',
      'number': '14429',
      'desc': 'Ayurveda, Yoga & natural medicine',
      'icon': Icons.spa,
      'color': _primary,
    },
    ...
    },
    {
      'name': 'Fire & Rescue',
      'number': '101',
      'desc': 'Fire brigade & rescue services',
    ...
    },
    {
      ...
      'number': '108',
      'desc': 'Natural disasters & calamities',
      'icon': Icons.flood,
      'color': _cardHelpline,
    },
    {
      'name': 'Road Accident',
      'number': '1073',
      'desc': 'Highway & road accident helpline',
      'icon': Icons.car_crash,
      'color': Color(0xFF6A3A8C),
    },
    {'category': 'Women & Children'},
    {
      'name': 'Women Helpline',
      'number': '1091',
      'desc': 'Women in distress & emergencies',
      'icon': Icons.female,
    ...
    {
      'name': 'Missing Child & Women',
      'number': '1094',
      'desc': 'Report missing persons',
      'icon': Icons.search,
      'color': _cardSOS,
    },
    {'category': 'Mental Health & Support'},
    {
      ...
      'color': Color(0xFF4A1580),
    },
    {
      'name': 'iCall',
      'number': '9152987821',
      'desc': 'Psychosocial support helpline',
      'icon': Icons.favorite,
      'color': Color(0xFFAD1457),
    },
    ...
    {'category': 'Other'},
    {
      ...
      'number': '139',
      'desc': 'Railway emergency & enquiry',
      'icon': Icons.train,
      'color': _cardHospital,
    },
    {
      'name': 'Anti Poison',
      'number': '1066',
      'desc': 'Poison control & drug overdose',
    ...
    },
    {
      ...
      'number': '1363',
      'desc': 'Help for tourists in India',
  ...
  Future<List<_Place>> _fetchOverpass(
    double userLat,
    double userLon,
    ...
    final filters = tagPairs
        .map(
          (kv) =>
              'node["${kv[0]}"="${kv[1]}"](around:$radius,$userLat,$userLon);'
              'way["${kv[0]}"="${kv[1]}"](around:$radius,$userLat,$userLon);',
        )
        .join('');
  ...
  Future<Position?> _getLocation({bool silent = false}) async {
    ...
    if (!await Geolocator.isLocationServiceEnabled()) {
      _setLocError('Location services disabled.');
      return null;
    }
    ...
    try {
      ...
      setState(() {
        _cachedPosition = pos;
        _locationText =
            '${pos.latitude.toStringAsFixed(5)}, ${pos.longitude.toStringAsFixed(5)} (±${pos.accuracy.toStringAsFixed(0)}m)';
        _locationBadge = 'LIVE';
        _badgeColor = _accent;
      });
      return pos;
    } catch (_) {
      _setLocError('Could not get location.');
      return null;
    }
  }
  ...
  // ── Smart call: dialog on web, dialer on Android/iOS ──────────────────────
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text(
                'On mobile, calls open directly in the dialer.',
                textAlign: TextAlign.center,
                style: GoogleFonts.urbanist(
                  color: _light,
                  fontSize: 11,
  ...
  Widget _placeListItem(_Place p, int i, Color numColor, bool isPolice) {
    ...
    if (ftLower == 'hospital')
      badge = 'Hospital';
    else if (ftLower == 'clinic')
      badge = 'Clinic';
    else if (ftLower == 'police')
      badge = 'Police';
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        '${(p.distance / 1000).toStringAsFixed(2)} km away',
                        style: GoogleFonts.urbanist(
                          fontSize: 12,
                          color: _light,
                          fontWeight: FontWeight.w500,
  ...
  // ── Send SOS — shows loading dialog immediately, then result ──────────────
  Future<void> _sendSOS() async {
    final pos = _cachedPosition ?? await _getLocation();
    if (pos == null) return;
    final lat = pos.latitude, lon = pos.longitude, acc = pos.accuracy;

    // ── Show loading dialog instantly so user knows something is happening ──
    if (!mounted) return;
    showDialog(
      context: context,
    ...
    try {
      final res = await http.post(
        Uri.parse('http://10.0.2.2:8000/send_location'),
      ...
      if (res.statusCode == 200) {
        ...
        place = d['place'] ?? place;
        mapsUrl = d['maps'] ?? mapsUrl;
      }
    } catch (_) {
      try {
        final nomRes = await http.get(
          Uri.parse(
            'https://nominatim.openstreetmap.org/reverse?lat=$lat&lon=$lon&zoom=18&format=json',
          ),
          headers: {'User-Agent': 'TravelShield-SOS-App'},
        );
        if (nomRes.statusCode == 200) {
          final d = jsonDecode(nomRes.body);
          place = d['name'] ?? d['display_name'] ?? place;
        }
      } catch (_) {}
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  _detailRow(
                    ...
                    _accent,
                    'Latitude',
                    lat.toStringAsFixed(8),
                  ),
                  _detailRow(
                    Icons.my_location_rounded,
                    _accent,
                    'Longitude',
                    lon.toStringAsFixed(8),
                  ),
        ...
        builder: (_, controller) => Column(
          children: [
            ...
            Container(
              ...
              padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 16),
              decoration: const BoxDecoration(
                gradient: LinearGradient(colors: [_primary, _cardHelpline]),
              ),
              child: Row(
                children: [
  ...
  Widget build(BuildContext context) {
    ...
    final contentW = screenW > 480 ? 420.0 : screenW;
              ...
              SliverAppBar(
                ...
                title: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Tap a button for immediate help',
                      style: GoogleFonts.urbanist(
                        fontSize: 12,
                        color: _light,
                        fontWeight: FontWeight.w500,
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  color: _accent.withOpacity(
                                    (1.8 - _pulseAnim.value).clamp(0.0, 1.0),
                                  ),
                                  width: 2,
                                ),
                              ),
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'On web, tap call buttons to see the number. Install the app on Android for direct dialing.',
                              style: GoogleFonts.urbanist(
                                fontSize: 11,
                                color: _primary,
                                fontWeight: FontWeight.w500,
