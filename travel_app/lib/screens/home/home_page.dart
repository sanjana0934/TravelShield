...
// Returns the same tip for the entire day, changes at midnight
String _getDailyTip() {
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: [
            ...
            // Gradient — darker on left for text legibility
            Positioned.fill(
              child: Container(
                decoration: BoxDecoration(
                    ...
                    Row(
                      children: [
                        const Icon(Icons.location_on_rounded,
                            color: Color(0xFF80E0AA), size: 12),
...
class _WeatherSafetyCardState extends State<_WeatherSafetyCard> {
  ...
  // Kerala district → lat/lng map for reverse lookup
  ...
  Future<void> _fetchWeather(String city) async {
    try {
      final res = await http
          .get(Uri.parse('$_baseUrl/clothing_suggestion/$city'))
                    ...
                    ? SizedBox(
                        ...
                        child: Center(
                          child: GestureDetector(
                            ...
                            child: Text(
                              '⚠️ $_error — Tap to retry',
                              style: GoogleFonts.urbanist(
                                  color: _light, fontSize: 13),
                            ),
                          ),
                        ),
                      )
